...
public class Calculator {
	...
	private void initialize() {
		...
		JButton btnDot = new JButton(".");			// 정수 실수로 Integer.parseint -> double.parsedouble
		...
		btnResult.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent e) {
		        double result = 0;
		        double currValue = Double.parseDouble(answer.getText()); // 사칙연산 버튼을 누른 후 0으로 초기화 되었으니 이후의 숫자는 currValue
		        switch(func) {			// 126번줄의 변경된 func에 따라 결과출력
		            case "ADD" : {
		...
		// 사칙연산 버튼들 : 사칙연산 버튼을 누르면 기존에 있던 숫자를 firstNumber에 저장 후 answer.setText("0"); 으로 화면 숫자 0으로 초기화후 func 변경
